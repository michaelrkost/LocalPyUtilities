...
import math
# from ib_insync import * #todo remove IB_insyc
...
def getMinMaxStockPricePercent(earningsPastStock, yahooEarningsDF, daysAroundEarnings=10):
    ...
    for count in range(0, len(yahooEarningsDF)):  # for all the dates in earningsCalendar
        ...
        onEarningDateForward = theEarningDate + datetime.timedelta(days=daysAroundEarnings)
...
def getHistoricStockPrices(stock, yahooEarningsDF, daysAroundEarnings = 10):
    ...
    for earnData in range(0, len(yahooEarningsDF)):
        ...
        endDateTime = dateUtils.getDateStringDashSeprtors(yahooEarningsDF['Earnings_Date'][earnData]+datetime.timedelta(days=daysAroundEarnings))
        startDateTime = dateUtils.getDateStringDashSeprtors(yahooEarningsDF['Earnings_Date'][earnData]-datetime.timedelta(days=daysAroundEarnings))
