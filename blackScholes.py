# black.py
# A generic Black-Scholes-Merton python calculator (MIT licence).
# The main routine (black) is a generic solver that returns the value of the missing variable (usually implied volatility, but could be price, yield, etc)
# ```
...
#  ```
# The library also has convenience functions such as `impliedBlack` that has volatility missing as an input, `blackScholes`
# that generates a forward by combining a dividend yield with the risk free rate and `impliedBlackScholes` which has the volatility input missing.

# Finally we have `impliedPair` which relies only on the strike price, time to maturity, risk-free and the prices of a call options and a put options at the strike.
# This function solves for both the risk-free forward and the risk-neutral implied-volatility for this strike (European options only).
# See Wilmott for further info: [Implied Volatility for same strike price for call and put](http://wilmott.com/messageview.cfm?catid=8&threadid=79623&FTVAR_MSGDBTABLE=)
